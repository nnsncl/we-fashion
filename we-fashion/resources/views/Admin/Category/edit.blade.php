...
@section('content')
    ...
    <div class="w-100 mb-16 " >
        <h1 class="text-3xl sm:text-5xl lg:text-6xl leading-none font-extrabold text-gray-900 tracking-tight mb-8" >Edit Category.</h1>
    </div>
    <form class="border-2 border-gray-100 p-6 bg-white rounded-xl w-100 mt-3 mb-3" action="{{ route('categories.update', $category->id ) }}" method="POST" >
        @csrf
        @method('PUT')
        <div class="flex flex-col-reverse md:flex-row justify-between flex-wrap items-start gap-3" >
            <div class="w-full flex justify-between flex-col mb-5" >
                <fieldset class="flex flex-col mb-3 ">
                    ...
                    <input class="text-lg outline-none py-3" type="text" name='gender' value="{{ $category->gender }}" placeholder="{{ $category->gender }}" />
                </fieldset>
            </div>
        </div>
